...
function OrganizerDashboardPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:8081/concerts/create', concertData, {
        headers: {
      ...
      <div className="form-container">
        <form onSubmit={handleSubmit}>
          ...
          <input
            ...
            required
          />
          <button type="submit" className="create-button" onClick={() => navigate('/')}>Create Concert</button>
        </form>
      </div>
    </div>
  );
